...
def search_videos(youtube,keyword,artist, song,maxResults=5):
    ...
    for record in response:
        if record["id"]["kind"] == "youtube#video":
            ...
            length = video_response.get("items", [])[0]["contentDetails"]["duration"].encode(encoding='UTF-8',errors='strict').lower()
            user_search = re.search("band|official|VEVO|records",user,
            flags=re.IGNORECASE)
            minute_search = re.search("pt([0-9]{1,}h)?([0-9]{1,2})m([0-9]{1,2})s",length,flags=re.IGNORECASE)
            if "rehearsal" not in title and "h" not in length and minute_search is not None:
    ...
    for j in range(0,MaxVideos):
        try:
            if j+inner_loop_index == TotalSongs:
                break
            artist = NewSongs.loc[j+inner_loop_index,["Artist"]].values[0]
            song = NewSongs.loc[j+inner_loop_index,["Song"]].values[0]
            results=search_videos(youtube,artist+' '+song,artist, song,maxResults=5)
            if results != "No results found":
                add_video_to_playlist(youtube,results['youtube_id'],playlist_id)
                AddedSongs.append(NewSongs.loc[j+inner_loop_index,["ID"]].values[0])
        except HttpError, e:
            MissedSongs = NewSongs[~NewSongs["ID"].isin(AddedSongs)]
            MissedSongs.to_csv(path_or_buf="MissedSongs_%d_%d.csv" % (i,inner_loop_index),index=False)
